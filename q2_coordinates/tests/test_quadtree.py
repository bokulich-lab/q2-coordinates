...
class BasicTest(unittest.TestCase):
        def setUp(self):
            ...
            self.test_df = test_df.set_index('SampleID')
            ...
            correct_dataframe = pd.DataFrame(correct_point, 
                columns=['SampleID', 'H1', 'H2'])
            self.correct_dataframe = correct_dataframe.set_index('SampleID')
        ...
        def test_clean_df(self):
            ...
            to_clean_dataframe = pd.DataFrame(incorrect_points, 
                columns=['SampleID', 'longitude', 'latitude'])
            to_clean_dataframe = to_clean_dataframe.set_index('SampleID')
            ...
            correct_cleaned_df = correct_cleaned_df.set_index('SampleID')
            ...
            cleaned =qtrees.clean(to_clean_dataframe, latitude='latitude', longitude='longitude', index='SampleID')
            ...
            str_only_df = pd.DataFrame(lat_long_str_pts,
                columns=['SampleID', 'longitude', 'latitude'])
            str_only_df = str_only_df.set_index('SampleID')


            with self.assertRaises(ValueError): 
                str_cleaned = qtrees.clean(str_only_df, 'latitude', 'longitude', 'SampleID')
            
        def test_get_results_outer(self):
            threshold = 2
            test_tree, test_samples = qtrees.get_results(self.test_df, threshold, index='SampleID')
        ...
        def test_threshold(self):
            threshold = 1
            with self.assertRaises(ValueError):
                tree_1, samples_1 = qtrees.get_results(self.test_df, threshold, index='SampleID')
            ...
            correct_depth1_df = pd.DataFrame(correct_depth1_pt,
                columns=['SampleID', 'H1'])
            correct_depth1_df = correct_depth1_df.set_index('SampleID')


            tree_4, samples_4 = qtrees.get_results(self.test_df, threshold, index='SampleID')
            pdt.assert_frame_equal(samples_4, correct_depth1_df)
            
            #what to do when threshold is higher than number of samples?
            threshold = 8
            tree_8, samples_8 = qtrees.get_results(self.test_df, threshold, index='SampleID')
